...
export function buildRestorePreview(
  ...
  const currentCreatureById = new Map(current.userCreatures.map((creature) => [creature.id, creature]));
  const incomingDiveById = new Map(incoming.dives.map((dive) => [dive.id, dive]));
  const incomingCreatureById = new Map(incoming.userCreatures.map((creature) => [creature.id, creature]));
  ...
  for (const creature of incoming.userCreatures) {
    ...
    else if (sameJsonValue(existing, creature)) mergeUnchangedUserCreatures += 1;
  }
  ...
  const replaceWouldDiscardUserCreatures = current.userCreatures.filter((creature) => {
...
export function applyPreparedRestore(
  ...
  if (conflicts.length > 0) {
    ...
    const suffix = conflicts.length > 3 ? `; plus ${conflicts.length - 3} more` : '';
  ...
  const currentCreatureIds = new Set(current.userCreatures.map((creature) => creature.id));
  const addedDives = incoming.dives.filter((dive) => !currentDiveIds.has(dive.id));
  const addedUserCreatures = incoming.userCreatures.filter((creature) => !currentCreatureIds.has(creature.id));
  const next: PersistedPersonalStateV2 = {
    schemaVersion: CURRENT_SCHEMA_VERSION,
    updatedAt: current.updatedAt,
    dives: [...deepClone(current.dives), ...deepClone(addedDives)],
    userCreatures: [...deepClone(current.userCreatures), ...deepClone(addedUserCreatures)],
  };
...
function assertExportEnvelope(raw: unknown): PoseidonExportV1 {
  if (!isRecord(raw)) throw new PoseidonValidationError('Backup is not a Poseidon JSON object.');
  if (raw.format !== 'poseidon-personal-export') {
    throw new PoseidonValidationError('Backup format is not poseidon-personal-export.');
  }
  ...
  if (!isRecord(raw.personal) || !Array.isArray(raw.personal.dives) || !Array.isArray(raw.personal.userCreatures)) {
  ...
  if (
    ...
    throw new PoseidonValidationError('Backup catalogue snapshot is malformed.');
  }
  return raw as unknown as PoseidonExportV1;
}

function validatePersonalState(state: PersistedPersonalStateV2, curatedCreatures: Creature[]): void {
  ...
  for (const creature of state.userCreatures) {
    ...
    if (curatedIds.has(creature.id)) {
      throw new PoseidonValidationError(`Backup custom creature ${creature.id} collides with the built-in catalogue.`);
    }
    const normalizedName = normalizeText(creature.commonName);
    if (normalizedUserNames.has(normalizedName)) {
      throw new PoseidonValidationError(`Backup contains duplicate custom creature name: ${creature.commonName}.`);
    }
...
function validateUserCreature(creature: Creature): void {
  ...
  if (record.curated !== false || record.userCreated !== true) {
    throw new PoseidonValidationError(`Custom creature ${String(record.id)} has invalid ownership metadata.`);
  }
  assertOptionalStringArray(record.aliases, `Custom creature ${String(record.id)} aliases`);
  assertOptionalString(record.scientificName, `Custom creature ${String(record.id)} scientificName`);
  assertOptionalString(record.category, `Custom creature ${String(record.id)} category`);
  assertOptionalStringArray(record.regionIds, `Custom creature ${String(record.id)} regionIds`);
  if (record.artwork !== undefined) {
    if (!isRecord(record.artwork) || !['curated', 'placeholder', 'missing'].includes(String(record.artwork.status))) {
      throw new PoseidonValidationError(`Custom creature ${String(record.id)} artwork metadata is malformed.`);
    }
  }
}
...
function validateDive(dive: Dive, availableCreatureIds: Set<string>): void {
  ...
  if (typeof record.date !== 'string') throw new PoseidonValidationError(`Dive ${String(record.id)} date is malformed.`);
  ...
  assertOptionalStringArray(record.buddies, `Dive ${String(record.id)} buddies`);

  if (record.countryCode !== undefined) {
    if (typeof record.countryCode !== 'string' || !/^[A-Za-z]{2}$/.test(record.countryCode)) {
  ...
  if (!Array.isArray(record.sightings)) {
    throw new PoseidonValidationError(`Dive ${String(record.id)} sightings are malformed.`);
  }
  const sightingIds = new Set<string>();
  const creatureIds = new Set<string>();
  for (const sighting of record.sightings) {
    if (!isRecord(sighting)) throw new PoseidonValidationError(`Dive ${String(record.id)} has a malformed sighting.`);
    const sightingId = assertNonBlankString(sighting.id, `Dive ${String(record.id)} sighting id`);
    const creatureId = assertNonBlankString(sighting.creatureId, `Dive ${String(record.id)} sighting creatureId`);
    if (sightingIds.has(sightingId)) {
      throw new PoseidonValidationError(`Dive ${String(record.id)} contains duplicate sighting id ${sightingId}.`);
    }
    if (creatureIds.has(creatureId)) {
      throw new PoseidonValidationError(`Dive ${String(record.id)} contains creature ${creatureId} more than once.`);
    }
    ...
    if (sighting.quantity !== undefined && !['one', 'few', 'several', 'many'].includes(String(sighting.quantity))) {
      throw new PoseidonValidationError(`Dive ${String(record.id)} has an unsupported sighting quantity.`);
    }
    assertOptionalString(sighting.note, `Dive ${String(record.id)} sighting note`);
    sightingIds.add(sightingId);
    creatureIds.add(creatureId);
  }
  if (record.highlightCreatureId !== undefined) {
    const highlight = assertNonBlankString(record.highlightCreatureId, `Dive ${String(record.id)} highlightCreatureId`);
    if (!creatureIds.has(highlight)) {
      throw new PoseidonValidationError(`Dive ${String(record.id)} highlightCreatureId is not one of its sightings.`);
    }
  }
}

function validateCatalogSnapshots(exportData: PoseidonExportV1): void {
  assertUniqueIds(exportData.catalogSnapshots.creatures, 'backup catalogue creatures');
  assertUniqueIds(exportData.catalogSnapshots.regions, 'backup catalogue regions');
  assertUniqueIds(exportData.catalogSnapshots.places, 'backup catalogue places');
  for (const creature of exportData.catalogSnapshots.creatures) {
    assertNonBlankString((creature as unknown as Record<string, unknown>).commonName, `Catalogue creature ${creature.id} name`);
  }
  for (const region of exportData.catalogSnapshots.regions) {
    assertNonBlankString((region as unknown as Record<string, unknown>).name, `Catalogue region ${region.id} name`);
  }
  for (const place of exportData.catalogSnapshots.places) {
    ...
    if (!['country', 'region', 'area', 'site'].includes(String(record.kind))) {
      throw new PoseidonValidationError(`Catalogue place ${place.id} kind is malformed.`);
    }
  }
}

function findMergeConflicts(current: PersistedPersonalStateV2, incoming: PersistedPersonalStateV2): string[] {
  const conflicts: string[] = [];
  const currentDiveById = new Map(current.dives.map((dive) => [dive.id, dive]));
  const currentCreatureById = new Map(current.userCreatures.map((creature) => [creature.id, creature]));
  const currentCreatureByName = new Map(current.userCreatures.map((creature) => [normalizeText(creature.commonName), creature]));

  for (const dive of incoming.dives) {
    const existing = currentDiveById.get(dive.id);
    if (existing && !sameJsonValue(existing, dive)) conflicts.push(`dive ${dive.id} has different data`);
  }
  for (const creature of incoming.userCreatures) {
    const existing = currentCreatureById.get(creature.id);
    ...
    const sameName = currentCreatureByName.get(normalizeText(creature.commonName));
    if (sameName && sameName.id !== creature.id) {
      conflicts.push(`custom creature name ${creature.commonName} already belongs to ${sameName.id}`);
    }
  }
  return conflicts;
}
...
function assertUniqueIds(records: Array<{ id: string }>, label: string): void {
  ...
  for (const raw of records) {
    const id = assertNonBlankString((raw as unknown as Record<string, unknown>).id, `${label} id`);
    if (ids.has(id)) throw new PoseidonValidationError(`Duplicate id ${id} in ${label}.`);
    ids.add(id);
  }
}

function assertNonBlankString(value: unknown, label: string): string {
  if (typeof value !== 'string' || !value.trim()) throw new PoseidonValidationError(`${label} must be a non-blank string.`);
  return value.trim();
}

function assertOptionalString(value: unknown, label: string): void {
  if (value !== undefined && typeof value !== 'string') throw new PoseidonValidationError(`${label} must be a string.`);
}
